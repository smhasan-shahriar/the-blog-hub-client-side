...
const BlogDetails = () => {
  ...
  const comments = useQuery({
    ...
    queryFn: () =>
      axios.get(`http://localhost:5000/comments/${id}`).then((res) => res.data),
  });
  ...
  const handlePostComment = e => {
    ...
    const userComment = {comment, blogId: id, userName: user.displayName, userImage: user.photoURL}
          ...
          {user && user?.email !== email ? (
            <div>
              <form onSubmit={handlePostComment}>
                  ...
                  <div className="w-full text-center mx-auto my-5"><input className="btn btn-primary normal-case text-lg font-bold" type="submit" value="Post Your Comment" /></div>
              </form>
            </div>
          ):<p className="text-xl font-bold text-left">You can not comment on your own blog</p>}
      </div>
      <div>
        {
            comments.data.map(comment => <CommentCard key={comment._id} userComment={comment}></CommentCard>)
        }
      </div>
      </div>
    </div>
